<template>
  <div class="tp-product-details-wrapper has-sticky">
    <div class="tp-product-details-category">
      <span>{{product.product_detail.product_name}}</span>
    </div>
    <h3 class="tp-product-details-title">{{product.product_detail.product_name}}</h3>
    ...
    <p>{{ product.product_detail.product_description }}</p>
    
    <!-- price -->
    <div class="tp-product-details-price-wrapper mb-20">
      <!-- <div v-if="Number(product.discount_percent.replace('%', '')) > 0">
          <span class="tp-product-details-price old-price">฿{{ product.fake_price }}</span>
          <span class="tp-product-details-price new-price">
            ฿{{(Number(product.fake_price) - (Number(product.fake_price) * Number(product.discount_percent.replace('%', ''))) / 100).toFixed(2)}}
          </span>
        </div> -->
        <div>
          <span class="tp-product-details-price old-price">฿{{ product.product_detail.fake_price }}</span>
          <span class="tp-product-details-price new-price">
            ฿{{product.product_detail.real_price}}
          </span>
        </div>
      <!-- <span class="tp-product-details-price new-price">฿{{ product.product_detail.fake_price }}</span> -->
    </div>
  ...
  <!-- product countdown end -->
      ...
      <div class="tp-product-details-action-item-wrapper d-flex align-items-center">
          <div class="tp-product-details-quantity">
            <div class="tp-product-quantity mb-15 mr-15">
                ...
                <input class="tp-cart-input" type="text" :value="1" disabled>
          ...
          <div class="tp-product-details-add-to-cart mb-15 w-100">
            ...
            <button class="tp-product-details-add-to-cart-btn w-100" @click="nextCartStore.addNextCartProduct(product)">Add To Cart</button>
          </div>
      </div>
      <nuxt-link :href="`/product-details/${product.product_detail.product_id}`" class="tp-product-details-buy-now-btn w-100 text-center">Buy Now</nuxt-link>
    </div>
    <div class="tp-product-details-action-sm">
      ...
      <button v-if="selectedPackage === 'L'" type="button" class="tp-product-details-action-sm-btn">
          <svg-compare-3/>
          Compare
      </button>
      <button type="button" class="tp-product-details-action-sm-btn">
    ...
    <div v-if="isShowBottom">
      ...
      <div class="tp-product-details-query-item d-flex align-items-center">
          <span>SKU:  </span>
          <p>{{product.product_detail.product_id}}</p>
      </div>
      <div class="tp-product-details-query-item d-flex align-items-center">
          <span>Category:  </span>
          <p>{{product.product_detail.product_name}}</p>
      </div>
      <div class="tp-product-details-query-item d-flex align-items-center">
          <span>Tag: </span>
          <p>Android</p>
...
// props
const props = withDefaults(defineProps<{product:NProductBody;isShowBottom?:boolean}>(), {
  isShowBottom:true,
})
