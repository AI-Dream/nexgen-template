 ...
 <div class="profile__address">
    <div class="row">
      <div class="col-xxl-12">
          <div class="profile__address-item d-sm-flex align-items-center">
            ...
            <div class="profile__address-content">
                <h3 class="profile__address-title mb-0">Shipping Address</h3>
            </div>
          </div>
      </div>
      <div class="col-xxl-12">
        <div class="profile__address_table table-responsive mt-25">
          <table v-if="addressStore.userAddress.length > 0" class="table">
            <thead>
                <tr>
            ...
            <tbody>
                <tr v-for="address, index in addressStore.userAddress">
                  <td style="max-width: 100px;">
                    {{ address.first_name }} 
                    {{ address.last_name }}
                  </td>
                  <td data-info="address" style="max-width: 150px;">
                    {{ address.detail }} 
                  ...
                  <td :data-info="address.default_address === 'Y' ? 'status done': '' " style="max-width: 20px;">{{ address.default_address === 'Y' ? 'ที่อยู่หลัก' : 'ที่อยู่สำรอง'}}</td>
                  <td style="max-width: 20px;">
                    <div class="d-flex gap-2">
                      <span @click="addressStore.openModal('shipping-update', address)">
                        <i class="fa-solid fa-pen-to-square"></i>
                      </span>
                      <span @click="addressStore.deleteShippingAddress(address.id)">
      ...
      <div class="col-xxl-12">
          <div class="profile__btn pt-25">
            <button type="submit" class="tp-btn" @click="addressStore.openModal('shipping-create')">Add Shipping Address</button>
          </div>
      </div>
      <!-- <div class="col-xxl-12 pt-55">
          <div class="profile__address-item d-sm-flex align-items-center">
            <div class="profile__address-icon">
                <span>
            ...
            <div class="profile__address-content">
                <h3 class="profile__address-title mb-0">Billing Address</h3>
            </div>
          </div>
      </div>
      <div class="col-xxl-12">        
          ...
          <table class="table">
            ...
            <tbody>
                <tr v-for="address, index in addressStore.userAddress">
                  ...
                  <td :data-info="address.default_address === 'Y' ? 'status done': '' " style="max-width: 20px;">{{ address.default_address === 'Y' ? 'ที่อยู่หลัก' : 'ที่่อยู่สำรอง'}}</td>
                  <td style="max-width: 20px;">
                    <div class="d-flex gap-2">
                      <span>
                        <i class="fa-solid fa-pen-to-square"></i>
      ...
      <div class="col-xxl-12">
          <div class="profile__btn pt-25">
            <button type="submit" class="tp-btn">Add Billing Address</button>
          </div>
      </div> -->
    </div>
</div>
<modal-shipping-create></modal-shipping-create>
<modal-shipping-update></modal-shipping-update>
</template>
...
const sortAddressWithDefaultFirst = (addresses) => {
  if (!addresses || !Array.isArray(addresses) || addresses.length <= 1) {
    return addresses
  }

  const sortedAddresses = [...addresses]
  const defaultIndex = sortedAddresses.findIndex(addr => addr.default_address === 'Y')
...
onMounted(() => {
  if (userStore.userData?.address_data) {
    userStore.userData.address_data = sortAddressWithDefaultFirst(userStore.userData.address_data)
  }
})
...
<style scoped>
.profile__address_table table thead tr {
  background-color: transparent;
}
.profile__address_table table th {
  color: var(--tp-common-black);
  border-color: #EAEAEF;
  box-shadow: none;
}
.profile__address_table table td {
  color: var(--tp-common-black);
}
.profile__address_table table tbody > tr:first-child {
  background-color: transparent;
}
.profile__address_table table tbody > tr:not(:last-child) {
  border-bottom: 1px solid #EAEAEF;
}
.profile__address_table table tbody > tr td, .profile__address_table table tbody > tr th {
  color: var(--tp-common-black);
  vertical-align: middle;
}
.profile__address_table table tbody > tr > th {
  padding-left: 15px;
  padding-right: 15px;
}
.profile__address_table table tbody > tr th[scope=row] {
  color: var(--tp-theme-primary);
}
.profile__address_table table tbody > tr td[data-info="status done"] {
  color: #08AF5C;
}
.profile__address_table table tbody > tr td[data-info="status reply"] {
  color: #D93D1E;
}
.profile__address_table table tbody > tr td[data-info="address"] {
  color: #6364DB;
}
.profile__address_table table tbody > tr td[data-info="status hold"] {
  color: #FFB422;
}
.profile__address_table table tbody > tr .tp-btn {
  padding: 7px 18px;
...
.profile__address_table table tbody > tr .tp-btn:hover {
  color: var(--tp-common-white);
  background-color: var(--tp-theme-primary);
}
.profile__address_table .table {
  margin-bottom: 0;
}
...
.profile__address_table .table > :not(:first-child) {
  border-color: #EAEAEF;
  border-top-width: 1px;
}
.profile__address_table .table > :not(caption) > * > * {
  padding: 1rem 1.5rem;
  border: 0;
  box-shadow: none;
}
